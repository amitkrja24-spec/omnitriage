...
function validateExtraction(data) {
  ...
  return {
    ...
    need_type: VALID_NEED_TYPES.includes(data.need_type)
      ? data.need_type
      : "other",
    urgency:
      Number.isInteger(data.urgency) && data.urgency >= 1 && data.urgency <= 5
        ? data.urgency
        : 3,
    skills_required: Array.isArray(data.skills_required)
      ...
      : ["general"],
    count_needed:
      Number.isInteger(data.count_needed) && data.count_needed > 0
        ? data.count_needed
        : 1,
    estimated_people_affected: Number.isInteger(data.estimated_people_affected)
      ? data.estimated_people_affected
      : null,
    brief_description:
      typeof data.brief_description === "string"
        ? data.brief_description.substring(0, 200)
        : "Field report submitted",
    confidence_score:
      typeof data.confidence_score === "number"
        ? Math.min(1.0, Math.max(0.0, data.confidence_score))
        : 0.5,
  };
}
